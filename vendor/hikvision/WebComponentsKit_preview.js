...
// 登录
function clickLogin(szIP, szPort, szUsername, szPassword,buttonId) {
	var szIP = szIP,
		szPort = szPort,
		...
		success: function (xmlDoc) {
			...
			setTimeout(function () {
				for (var i = 1; i <= channels_; i++) {
					clickStartRealPlay(szIP, i,buttonId);
				}
			}, 1000);
		},
		error: function () {
			layer.tips('登陆失败，请刷新页面重试', buttonId, { tips: [1, '#FF5722'], });
		}
	});
 ...
 * @param {*} i 门禁对应的摄像头通道
 */
function door_clickLogin(szIP, szPort, szUsername, szPassword,door_camera_index,buttonId) {
	var szIP = szIP,
		szPort = szPort,
		szUsername = szUsername,
		szPassword = szPassword;
	...
	var iRet = WebVideoCtrl.I_Login(szIP, 1, szPort, szUsername, szPassword, {
		success: function (xmlDoc) {
			...
			setTimeout(function() {
				clickStartRealPlay(szIP, door_camera_index,buttonId);
			}, 1000);
		},
		error: function () {
			layer.tips('登陆失败，请刷新页面重试', buttonId, { tips: [1, '#FF5722'], });
...
// 开始预览
function clickStartRealPlay(szIP, iChannelID,buttonId) {
	// var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
	var szIP = szIP, //摄像头ip
		iStreamType = 1, //码流类型
		iChannelID = iChannelID, //播放的通道ID 默认通道1
		szInfo = "";
	...
	var iRet = WebVideoCtrl.I_StartRealPlay(szIP, {
		iWndIndex: iChannelID - 1, //播放窗口 从0开始
		iStreamType: iStreamType, //码流类型
		iChannelID: iChannelID //通道ID
	});
	...
	// 	szInfo = "开始预览失败！";
	// }
	layer.tips("开启预览成功", buttonId, { tips: [1, '#009688'], });
	// showOPInfo(szIP + " " + szInfo);
}
